...
if ! RUNTIME_IMAGES=$(
    ...
    #docker compose "${COMPOSE_ARGS[@]}" config --images | sort -u
     raw_output=$(podman compose "${COMPOSE_ARGS[@]}" images | sort -u)

    # 判断第一行是否含 "CONTAINER"（表头特征），若是则跳过表头，提取第2列和第3列组合为 "REPOSITORY:TAG"
    if echo "$raw_output" | head -n1 | grep -q "CONTAINER"; then
        echo "$raw_output" | tail -n +2 | awk '{print $2":"$3}' | sort -u
    else
        echo "$raw_output" | sort -u
    fi
); then
...
log_info "starting stack: podman compose ${COMPOSE_ARGS[*]} up -d"
(
    cd "$INSTALL_DIR"
